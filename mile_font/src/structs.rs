...
// use crate::{event::{BatchFontEntry, BatchRenderFont}, prelude::{CpuText, GpuChar, GpuText}};
...
//     fonts_texture_map:HashMap<Arc<char>, u32>,
...
//     register_global_event: RegisterEvent,
//     store: RenderPlanStore,
//     quad_index:u32,
//     gpu_chars_vec:GpuCharsManager
// }
...
// struct GpuCharsManager{
//     gpu_chars:[GpuChar;1024],
//     update_range:Vec<GpuRange>,
//     new_idle_range:Vec<GpuRange>,
// }

// impl GpuCharsManager{
//     fn input(&mut self,plan:CpuText){
//         let idle_count = self.new_idle_range
//         if(!self.new_idle_range.is_empty()){
//             plan.slice.iter().take(n)
//         }
//     }
// }
...
//         let (batch_entry_fonts,batch_render_fonts) = self.register_global_event.poll();

//     if batch_entry_fonts.is_empty() && batch_render_fonts.is_empty() {
//         return;
//     }

        
//     // ① 改成 HashMap<Arc<str>, Vec<Arc<str>>>
//     let mut grouped: HashMap<Arc<str>, Vec<char>> = HashMap::new();

//     // ② 分组（Clone Arc 是 O(1)，不会复制字符串）
//     for e in &batch_entry_fonts {
//         for s in e.text.chars(){
//             grouped
//                 .entry(e.font_file_path.clone())
//                 .or_default()
//                 .push(s.clone());
//             println!("当前状况 {:?}",s);
//         }
...
//     // ③ 遍历 grouped
//     for (font_file_name, texts) in grouped {
//         let mut cached: Vec<(char, u32)> = Vec::new();
//         let mut uncached: HashSet<char> = HashSet::new();

//         for text in texts.iter() {
//             match self.fonts_texture_map.get(text) {
//                 Some(map) => cached.push((text.clone(), map.clone())),
//                 None => {
//                     uncached.insert(text.clone());
//                     ()
//                 },
//             }
//         }

//         let result = self.queue_batch_parse(font_file_name, uncached, 16);

//         if(cached.len() > 0){
//             println!("当前的缓存字体情况 {:?}",cached);
//         }

//         let cache = self.cache.generic_buffer_cache.as_mut().unwrap();

//         if let Ok(mut res) = result {
//             println!("加入字体到印刷板 {:?}", texts);

//             let (des, instruction) = res.to_gpu_struct(cache.instruction_buffer_index);
//             let start_index = self.write_batch_buffer(queue, &des, &instruction);
//             for (index,font_item) in des.iter().enumerate(){
//                 self.fonts_texture_map.insert(Arc::from(res.results[index].character),start_index + index as u32);
//                 println!("当前字符有效的为 {:?} {:?}",res.results[index].character,start_index + index as u32);
//             }
//         }


//         //渲染字体
//         for font_render in &batch_render_fonts{
//             let quad_start_index = self.quad_index;
//             let gpu_chars = font_render.text.chars()
//             .enumerate()
//              .map(|(arr_index,e)| {
//                 if let Some(index) = self.fonts_texture_map.get(&e){
//                     Some(GpuChar{
//                         char_index: *index,
//                         quad_index: quad_start_index + arr_index as u32,
//                         panel_index: font_render.parent.0,
//                         _pad: 0,
//                     })
//                 }else {
//                     None
...
//              let plan = CpuText{
//                 raw: font_render.text.clone(),
//                 slice: gpu_chars,
//                 link_panel_quad: font_render.parent.clone(),
//             };

//             println!("渲染的字符串结构为 {:?}",plan);
//         }
...
//         // let result = self.queue_batch_parse(Arc::from("../ttf/BIZUDPGothic-Regular.ttf"), Arc::from(["币"]), 16);
//         // let cache = self.cache.generic_buffer_cache.as_mut().unwrap();
...
//     ) ->u32 {
//         let cache = self.cache.generic_buffer_cache.as_mut().unwrap();
//         let index = cache.instruction_buffer_index as u32;
//         let ins_offset =
...
//     {
//         let texts = text_batch.into_iter().collect::<Vec<_>>();
//         // --- Step 1: 获取字体句柄 ---
...
//         let new_results: Vec<FontGlyphResult> = texts
//             .par_iter()
//             .flat_map(|ch| {
