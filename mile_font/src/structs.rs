use std::{cell::RefCell, collections::HashMap, error::Error, fs, hash::Hash, io, path::{Path, PathBuf}, rc::Rc, sync::{Arc, Mutex}};

use bytemuck::{Pod, Zeroable};
use mile_api::{Computeable, CpuGlobalUniform, GpuDebug, Renderable};
use rayon::iter::{IntoParallelRefIterator, ParallelIterator};
use ttf_parser::{morx::InsertionEntryData, Face, GlyphId, OutlineBuilder};
use wgpu::{hal::{auxil::db, TextureDescriptor}, util::DeviceExt, Buffer, BufferUsages, Extent3d, RenderPass, SamplerBindingType, TextureFormat, TextureViewDescriptor};
...
impl Renderable for MileFont{
    fn render<'a>(&self, device: &wgpu::Device, queue: &wgpu::Queue, view: &wgpu::TextureView,pass:&mut RenderPass<'a>) {

        {
            ...
            pass.set_bind_group(0, &self.cache.render_bind_cache.as_ref().unwrap().render_bind_group, &[]);
            pass.set_vertex_buffer(0, self.cache.render_buffer_cache.as_ref().unwrap().vertex_buffer.slice(..));
            pass.set_index_buffer(self.cache.render_buffer_cache.as_ref().unwrap().index_buffer.slice(..), wgpu::IndexFormat::Uint16);
            pass.draw_indexed(0..QUAD_INDICES.len() as u32, 0, 0..self.num_instances as u32);
        }
...
impl FontBatch {
pub fn to_gpu_struct(&mut self, start_offset: u64) -> (Vec<FontGlyphDes>, Vec<GlyphInstruction>) {
        ...
        for glyph in &self.results {
            ...
            println!("当前加入字形 {:?} gpu:start{} gpu:end{}",glyph.character,start_idx,end_idx);
...
impl MileFont{   
   ...
   pub fn new(global_unifrom:Rc<CpuGlobalUniform>)->Self{
        Self { gpu_debug:RefCell::new(GpuDebug::new("MileFont"))
   ...
   pub fn test_entry_text(&mut self,queue:&wgpu::Queue){
        let vertex_buffer = self.cache.render_buffer_cache.as_ref().unwrap().vertex_buffer.clone();
        let index_buffer = self.cache.render_buffer_cache.as_ref().unwrap().index_buffer.clone();
   ...
   pub fn write_batch_buffer(&mut self,queue:&wgpu::Queue,fond_des:&[FontGlyphDes], instruction:&[GlyphInstruction]){
        let cache =  self.cache.generic_buffer_cache.as_mut().unwrap();
        let ins_offset = cache.instruction_buffer_index * std::mem::size_of::<GlyphInstruction>() as u64;
        let des_offset = cache.instruction_buffer_index  as u64 * std::mem::size_of::<FontGlyphDes>() as u64;
   ...
   pub fn batch_enqueue_compute(&self,device:&wgpu::Device,queue:&wgpu::Queue){
        ...
        
        {
            ...
            cpass.set_bind_group(0, &self.cache.compute_bind_cache.as_ref().unwrap().bind_0_group, &[]);
            let workgroups = 1;
    ...
    pub fn copy_store_texture_to_render_texture(
        ...
        println!("拷贝sdf纹理到render width:{:?} height:{:?}",atlas_width,atlas_height);
   ...
   pub fn create_render_pipeline(&mut self,device:&wgpu::Device,format:TextureFormat){
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            ...
            bind_group_layouts: &[
                self.cache.render_bind_cache.as_ref().unwrap().render_bind_group_layout.as_ref().unwrap(),
            ],
      ...
      let compute_shader_module = device.create_shader_module(wgpu::ShaderModuleDescriptor {
        ...
        let compute_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
                    resource: wgpu::BindingResource::TextureView(&self.cache.generic_buffer_cache.as_ref().unwrap().storage_texture_view),
                },
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: generic_buffer_cache.instruction_buffer.as_ref().unwrap().as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: generic_buffer_cache.instruction_des_buffer.as_ref().unwrap().as_entire_binding(),
                },
                wgpu::BindGroupEntry {
                    binding: 3,
        ...
        self.cache.compute_bind_cache = Some(ComputeBindCache { bind_0_group: Some(compute_bind_group) });
        self.cache.compute_pipeline = Some(batch_enqueue_compute_pipeline);

   }

   pub fn create_template_render_texture_and_layout(&mut self,device:&wgpu::Device,format: Option<wgpu::TextureFormat>){
        ...
        let group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            ...
            wgpu::BindGroupEntry {
                binding: 0,
                resource: wgpu::BindingResource::TextureView(&self.cache.generic_buffer_cache.as_ref().unwrap().render_texture_view),
            },            
            wgpu::BindGroupEntry {
                binding: 1,
                resource: wgpu::BindingResource::Sampler(&self.cache.generic_buffer_cache.as_ref().unwrap().sampler)
            },      
            wgpu::BindGroupEntry {
                binding: 2,
                resource: self.cache.generic_buffer_cache.as_ref().unwrap().instruction_buffer.as_ref().unwrap().as_entire_binding()
            },
            wgpu::BindGroupEntry {
                binding: 3,
        ...
        self.cache.render_bind_cache = Some(RenderBindCache { render_bind_group_layout: Some(bind_group_layout), render_bind_group: Some(group) })
   }
   ...
   pub fn init_buffer(&mut self,device:&wgpu::Device){
        ...
        let storage_texture = device.create_texture(&wgpu::TextureDescriptor {
            ...
            size: Extent3d { width: 4096, height: 4096, depth_or_array_layers: 1 },
            mip_level_count: 1,
            sample_count: 1,
...
// 确认它们是同一个对象
  ...
  pub fn queue_batch_parse(
    ...
    // --- Step 2: 获取或创建缓存 ---
    let cache = self.entries.entry(key.clone()).or_insert_with(|| FontCacheEntry {
        key: key.clone(),
        glyphs: HashMap::new(),
        last_used_frame: 0,
    ...
    let new_results: Vec<FontGlyphResult> = text_batch
        ...
        let metrics = GlyphMetrics {
            ...
            left_side_bearing: face.glyph_hor_side_bearing(glyph_id).unwrap_or(0) as i32,
            ...
            glyph_left_side_bearing: face.glyph_hor_side_bearing(glyph_id).unwrap_or(0) as i32,
            
            // 垂直度量（如果需要）
            glyph_ver_advance: face.glyph_ver_advance(glyph_id).unwrap_or(0) as u32,
            glyph_ver_side_bearing: face.glyph_ver_side_bearing(glyph_id).unwrap_or(0) as i32,
...
impl OutlineBuilder for GlyphBuilder {
    ...
    fn quad_to(&mut self, x1: f32, y1: f32, x: f32, y: f32) {
        self.instructions.push(GlyphInstruction::quad_to(x1, y1, x, y));
    }
    fn curve_to(&mut self, x1: f32, y1: f32, x2: f32, y2: f32, x: f32, y: f32) {
        self.instructions.push(GlyphInstruction::curve_to(x1, y1, x2, y2, x, y));
    }
    fn close(&mut self) {
