...
impl NetworkStore {
    pub fn add_rel(&mut self, rel: Rel) {
        ...
        for panel in self.panels_ref.values_mut() {
            if panel
                ...
                .contains_key(&rel.source_collection.clone())
            {
        ...
        println!(
            ...
            self.panels_ref
                ...
                .filter(|p| p
                    .collection_index_map
                    .contains_key(&rel.clone().source_collection.clone()))
                .count()
        );
    }
        ...
        if let Some(buffer_collection) = &self.buffer_meta {
            for &collection_id in &self.update_collection_sign {
                if let Some(collection) = self.collections.get(&collection_id) {
                    ...
                    self.buffer_sim[offset..offset + write_len]
                        .copy_from_slice(&collection.children[..write_len]);
                    ...
                    queue.write_buffer(
                        ...
                        bytemuck::cast_slice(
                            &self.buffer_sim[offset..offset + MAX_COLLECTION_ITEMS],
                        ),
                    );
                }
            }
    ...
    pub fn add_panel_to_collection(&mut self, panel_id: PanelId, collection_id: CollectionId) {
        ...
        // --- 获取或创建 panel ---
        let panel = self.panels_ref.entry(panel_id).or_insert_with(|| PanelRef {
            id: panel_id,
            collection_index_map: HashMap::new(),
            rel_index_map: vec![],
            dirty: true,
        });
        ...
        self.buffer_sim[base + COLLECTION_META_OFFSET + 1] =
            (collection.children_offset + 128) as u32;
    ...
    pub fn upload_dirty_to_gpu_batch(&mut self, queue: &wgpu::Queue) {
        ...
        if !dirty_panels.is_empty() {
            ...
            let meta_end =
                dirty_panels.last().unwrap().id as usize * PANEL_META_SIZE + PANEL_META_SIZE;
            if let Some(buffer_meta) = &self.buffer_meta {
                println!(
                    "Meta upload for panels {}..{}: start={} end={}",
                    dirty_panels.first().unwrap().id,
                    dirty_panels.last().unwrap().id,
                    meta_start,
                    meta_end
                );
                queue.write_buffer(
                    buffer_meta,
                    0,
                    bytemuck::cast_slice(&self.buffer_sim[meta_start..meta_end]),
                );
            }
        }
        ...
        if self.last_uploaded_rel_end < self.rel_flat_next {
            if let Some(buffer_rels) = &self.buffer_rels {
                let rel_slice = &self.buffer_sim[(self.last_uploaded_rel_end + REL_SIM_OFFSET)
                    ..(self.rel_flat_next + REL_SIM_OFFSET)];
                assert!(rel_slice.len() % 8 == 0, "Rel 数据必须是 4 的倍数");
                let rel_u32_count = rel_slice.len();
                println!(
                    "Rels upload: start={} end={} data={:?}",
                    self.last_uploaded_rel_end, self.rel_flat_next, rel_slice
                );
                queue.write_buffer(
                    buffer_rels,
                    (self.last_uploaded_rel_end) as wgpu::BufferAddress,
                    bytemuck::cast_slice(rel_slice),
                );
            }
