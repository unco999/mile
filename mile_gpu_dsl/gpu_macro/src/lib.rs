...
fn lex(s: &str) -> Vec<Tok> {
    ...
    while i < n {
        ...
        if c.is_ascii_digit() || (c == '.' && i+1 < n && s[i+1..].chars().next().unwrap().is_ascii_digit()) {
            ...
            if let Ok(v) = num_str.parse::<f32>() {
                ...
                let trimmed = num_str.trim_end_matches(|ch: char| !ch.is_numeric() && ch != '.' && ch != 'e' && ch != 'E' && ch != '+' && ch != '-');
        ...
        if i+1 < n {
            let two = &s[i..i+2];
            match two {
                ">=" | "<=" | "==" | "!=" => { out.push(Tok::Op(two.to_string())); i += 2; continue; }
                _ => {}
            }
        }
        match c {
            ...
            _ => { // unknown - treat as ident char sequence
                let mut j = i+ c.len_utf8();
                while j < n {
                    let cc = s[j..].chars().next().unwrap();
                    if cc.is_whitespace() || cc=='('||cc==')'||cc==',' { break; }
                    j += cc.len_utf8();
                }
                let chunk = &s[i..j];
...
enum PExpr {
    Const(f32),
    Var(String),
    Binary { op: String, a: Box<PExpr>, b: Box<PExpr> },
    UnaryFunc { name: String, a: Box<PExpr> },
...
impl Parser {
    ...
    fn expect_ident(&mut self) -> Result<String, String> {
        match self.bump() { Tok::Ident(s) => Ok(s), t => Err(format!("expected ident, got {:?}", t)) }
    }

    fn parse(&mut self) -> Result<PExpr, String> { self.parse_expr() }

    // expr -> cmp
    fn parse_expr(&mut self) -> Result<PExpr, String> { self.parse_cmp() }
        ...
        loop {
            match self.peek() {
                Tok::Op(op) if ["==","!=",">=","<=",">","<"].contains(&op.as_str()) => {
                    let op = if let Tok::Op(s)=self.bump(){s}else{"".to_string()};
                    let right = self.parse_add()?;
                    node = PExpr::Binary { op, a: Box::new(node), b: Box::new(right) };
                }
                _ => break,
            }
        }
    ...
    fn parse_unary(&mut self) -> Result<PExpr, String> {
        match self.peek() {
            Tok::Op(op) if op=="-" => { self.bump(); let v = self.parse_unary()?; Ok(PExpr::Binary { op: "*".to_string(), a: Box::new(PExpr::Const(-1.0)), b: Box::new(v) }) }
            Tok::Op(op) if op=="+" => { self.bump(); self.parse_unary() }
    ...
    fn parse_primary(&mut self) -> Result<PExpr, String> {
        match self.peek().clone() {
            Tok::Number(v) => { self.bump(); Ok(PExpr::Const(v)) }
            Tok::Ident(ref s) => {
                ...
                match self.peek() {
                    Tok::LParen => {
                        ...
                        if let Tok::RParen = self.peek() { self.bump(); } else {
                            loop {
                                let e = self.parse_expr()?;
                                args.push(e);
                                match self.peek() {
                                    Tok::Comma => { self.bump(); continue; }
                                    Tok::RParen => { self.bump(); break; }
                        ...
                        match name.as_str() {
                            "vec2" => {
                                if args.len()!=2 { return Err("vec2 expects 2 args".into()); }
                                Ok(PExpr::Vec2(Box::new(args.remove(0)), Box::new(args.remove(0))))
                            }
                            "vec3" => {
                                if args.len()!=3 { return Err("vec3 expects 3 args".into()); }
                                Ok(PExpr::Vec3(Box::new(args.remove(0)), Box::new(args.remove(0)), Box::new(args.remove(0))))
                            }
                            "vec4" => {
                                if args.len()!=4 { return Err("vec4 expects 4 args".into()); }
                                Ok(PExpr::Vec4(Box::new(args.remove(0)), Box::new(args.remove(0)), Box::new(args.remove(0)), Box::new(args.remove(0))))
                            }
                            "if" => {
                                if args.len()!=3 { return Err("if(cond,then,else) expects 3 args".into()); }
                                Ok(PExpr::If(Box::new(args.remove(0)), Box::new(args.remove(0)), Box::new(args.remove(0))))
                            }
                            "sin" | "cos" | "sqrt" | "tan" | "exp" | "log" | "abs" => {
                                if args.len()!=1 { return Err(format!("{} expects 1 arg", name)); }
                                Ok(PExpr::UnaryFunc { name, a: Box::new(args.remove(0)) })
                            }
                            _ => {
            ...
            Tok::LParen => {
                self.bump();
                let e = self.parse_expr()?;
                match self.peek() { Tok::RParen => { self.bump(); Ok(e) } _ => Err("expected )".into()) }
            }
...
impl PExpr {
    fn to_tokenstream(&self) -> proc_macro2::TokenStream {
        match self {
            ...
            PExpr::Vec2(x,y) => {
                let xs = x.to_tokenstream(); let ys = y.to_tokenstream();
                quote! { crate::test::Expr::Vec2(crate::test::Vec2::new(Box::new(#xs), Box::new(#ys))) }
            }
            PExpr::Vec3(x,y,z) => {
                let xs = x.to_tokenstream(); let ys = y.to_tokenstream(); let zs = z.to_tokenstream();
                quote! { crate::test::Expr::Vec3(crate::test::Vec3::new(Box::new(#xs), Box::new(#ys), Box::new(#zs))) }
            }
            PExpr::Vec4(a,b,c,d) => {
                let a1 = a.to_tokenstream(); let b1 = b.to_tokenstream(); let c1 = c.to_tokenstream(); let d1 = d.to_tokenstream();
                quote! { crate::test::Expr::Vec4(crate::test::Vec4::new(Box::new(#a1), Box::new(#b1), Box::new(#c1), Box::new(#d1))) }
            }
            PExpr::If(c,t,e) => {
                let cts = c.to_tokenstream(); let tts = t.to_tokenstream(); let ets = e.to_tokenstream();
